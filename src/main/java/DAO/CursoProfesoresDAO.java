...
public class CursoProfesoresDAO {
    ...
    public CursoProfesor buscarCursoProfesor(int profesorID, int cursoID) {
        ...
        try (
            ...
            if (rs.next()) {
                return FabricaModelo.crearCursoProfesor(
...
public void actualizar(int profesorID, int cursoID, int nuevoAnio, int nuevoSemestre) {
        ...
        try (
            ...
            while (rs.next()) {
                ...
                if (profesor != null && curso != null) {
                    listado.add(FabricaModelo.crearCursoProfesor(profesor, anio, semestre, curso));
                }
            }
        } catch (SQLException e) {
   ...
   private Curso obtenerCursoPorID(int cursoID, Connection conexion) throws SQLException {
    ...
    try (PreparedStatement pstmt = conexion.prepareStatement(sql)) {
        ...
        ResultSet rs = pstmt.executeQuery();
        if (rs.next()) {
            Facultad facultad = FabricaModelo.crearFacultad(rs.getInt("facultad_id"), rs.getString("facultad_nombre"), null);
            Programa programa = FabricaModelo.crearPrograma(rs.getInt("programa_id"), rs.getString("programa_nombre"), 
                                             rs.getDouble("duracion"), rs.getDate("registro"), facultad);
            return FabricaModelo.crearCurso(rs.getInt("id"), rs.getString("nombre"), programa, rs.getBoolean("activo"));
        }
    }
    return null;
}
